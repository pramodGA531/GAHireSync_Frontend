...
const FillJobDetails = ({
    ...
    const [conversationState, setConversationState] = useState("idle"); // idle, processing, confirming, asking
    const [extractedData, setExtractedData] = useState(null);
    const [isAIProcessing, setIsAIProcessing] = useState(false);
    const [pendingAction, setPendingAction] = useState(null);

    // Log chat panel state changes
    useEffect(() => {
        console.log("Chat Panel State:", {
            timestamp: new Date().toISOString(),
            status: isChatExpanded ? "OPEN" : "CLOSED",
        });
    }, [isChatExpanded]);
    ...
    // --- AI Chat Helper Components ---
    const RadioOptionItem = ({ msg, onSelect }) => {
        return (
            <div className="flex flex-col gap-2">
                ...
                <div className="ml-2 bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                    <Radio.Group
                        ...
                        onChange={(e) =>
                            onSelect(e.target.value, msg.fieldName)
                        }
                    >
                        {msg.options.map((option, idx) => (
                            <Radio
                                key={idx}
                                value={option}
                                className="px-4 py-3 hover:bg-blue-50 transition-colors border-b last:border-b-0 border-gray-100 w-full"
                            >
    ...
    const CheckboxOptionItem = ({ msg, onSelect }) => {
                ...
                <div className="ml-2 bg-white rounded-lg border border-gray-200 overflow-hidden shadow-sm">
                    <div className="flex flex-col max-h-64 overflow-y-auto">
                        {msg.options.map((option, idx) => (
                            <div
                                key={idx}
                                className="flex items-center gap-3 px-4 py-3 hover:bg-blue-50 cursor-pointer border-b last:border-b-0 border-gray-100 transition-colors"
                            ...
                            >
                                <input
                                    ...
                                    readOnly
                                    className="w-4 h-4 px-2 py-1 text-blue-500 rounded focus:ring-blue-400"
                                />
                                <span className="text-gray-700">{option}</span>
                            </div>
                        ))}
                        <div className="p-3 bg-gray-50 border-t border-gray-100 flex justify-end">
                            <Button
                                ...
                                onClick={() =>
                                    onSelect(selected, msg.fieldName)
                                }
                            >
                                Confirm Selection
                            </Button>
    ...
    const SearchableSelectItem = ({ msg, onSelect }) => {
        return (
            <div className="flex flex-col gap-2">
                <div className="flex justify-start">
                    <div className="bg-gray-100 rounded-lg p-3 max-w-[80%] border border-gray-200">
                ...
                <div className="ml-2 bg-white rounded-lg border border-gray-200 p-3 shadow-sm">
                    <Select
                        showSearch
                        placeholder="Search and select..."
                        className="w-full"
                        optionFilterProp="children"
                        onChange={(value) => onSelect(value, msg.fieldName)}
    ...
    const CalendarItem = ({ msg, onSelect }) => {
        ...
        return (
            <div className="flex flex-col gap-2">
                ...
                <div className="ml-2 flex flex-col gap-2 items-start">
                    <CustomDatePicker
                        value={selectedDate}
                        onChange={(date) => {
                            const dateStr = date
                    ...
                    <button
                        ...
                        className="bg-blue-500 text-white px-4 py-1.5 rounded hover:bg-blue-600 transition-colors text-sm font-semibold disabled:bg-gray-300"
                    >
                        Confirm Date
                    </button>
                </div>
    ...
    // Helper function to add confirmation message
    const addConfirmationMessage = (text, onYes, onNo) => {
        const confirmMsg = {
            id: Date.now(),
    ...
    // Helper function to add radio options message
    const addRadioOptionsMessage = (question, options, fieldName) => {
        const radioMsg = {
            id: Date.now(),
            type: MESSAGE_TYPES.RADIO_OPTIONS,
            question,
            options,
            fieldName,
            timestamp: new Date().toISOString(),
        };
        setMessages((prev) => [...prev, radioMsg]);
    };

    // Helper function to add checkbox options message
    const addCheckboxOptionsMessage = (question, options, fieldName) => {
        const checkboxMsg = {
            id: Date.now(),
            type: MESSAGE_TYPES.CHECKBOX_OPTIONS,
            question,
            options,
            fieldName,
            timestamp: new Date().toISOString(),
        };
        setMessages((prev) => [...prev, checkboxMsg]);
    };

    // Helper function to add calendar message
    const addCalendarMessage = (question, fieldName) => {
        const calendarMsg = {
            id: Date.now(),
        ...
        setMessages((prev) => [...prev, calendarMsg]);
    };

    // Helper function to add searchable select message
    const addSearchSelectMessage = (question, options, fieldName) => {
        const searchSelectMsg = {
            id: Date.now(),
            type: MESSAGE_TYPES.SEARCH_SELECT,
            question,
            options,
            fieldName,
            timestamp: new Date().toISOString(),
        };
        setMessages((prev) => [...prev, searchSelectMsg]);
    };

    // Helper function to add form preview
    const addFormPreviewMessage = (data) => {
        const previewMsg = {
            id: Date.now(),
            type: MESSAGE_TYPES.FORM_PREVIEW,
            data,
            timestamp: new Date().toISOString(),
        };
    ...
    const fillFormFields = (data) => {
        if (!data) return;
        console.log(
            "🤖 AI Chatbot - START: Filling form with extracted data:",
            data,
        );

        // Track updates to sync with parent setFormValues
        const updates = {};

        // Accumulate range updates to avoid race conditions
        const currentExp = watch("years_of_experience");
        const currentCtc = watch("ctc");
        let expUpdate = Array.isArray(currentExp)
            ? [...currentExp]
            : [null, null];
        let ctcUpdate = Array.isArray(currentCtc)
            ? [...currentCtc]
            : [null, null];
        let hasExpUpdate = false;
        let hasCtcUpdate = false;

        Object.entries(data).forEach(([key, value]) => {
            // Robust normalization: camelCase to snake_case, spaces to underscores
            ...
            // Explicit mappings for common AI variants
            const keyMap = {
                // Job Title
                job_title: "job_title",
                jobtitle: "job_title",
                ...
                // CTC Min/Max - Individual questions
                ctc_min: "ctc_min",
                ctcmin: "ctc_min",
                minimum_ctc: "ctc_min",
                minimumctc: "ctc_min",
                ...
                ctc_maximum: "ctc_max",
                salary_max: "ctc_max",
                maximum_salary: "ctc_max",

                // Experience Min/Max - Individual questions
                experience_min: "experience_min",
                experiencemin: "experience_min",
                minimum_experience: "experience_min",
                minimumexperience: "experience_min",
                min_experience: "experience_min",
                minexperience: "experience_min",
                experience_minimum: "experience_min",
                min_years_of_experience: "experience_min",
                minimum_years_of_experience: "experience_min",

                experience_max: "experience_max",
                experiencemax: "experience_max",
                maximum_experience: "experience_max",
                maximumexperience: "experience_max",
                max_experience: "experience_max",
                maxexperience: "experience_max",
                experience_maximum: "experience_max",
                max_years_of_experience: "experience_max",
                maximum_years_of_experience: "experience_max",
                years_of_experience_min: "experience_min",
                years_of_experience_max: "experience_max",
                ...
                // Passport
                passport_availability: "passport_availability",
                passportavailability: "passport_availability",
                passport: "passport_availability",
                ...
                perks: "other_benefits",

                // Decision Maker
                decision_maker: "decision_maker",
                decisionmaker: "decision_maker",
                hiring_manager: "decision_maker",
                hiringmanager: "decision_maker",

                // Decision Maker Email
                decision_maker_email: "decision_maker_email",
                decisionmakeremail: "decision_maker_email",
                hiring_manager_email: "decision_maker_email",
                ...
                rotationalshift: "rotational_shift",
                shift: "rotational_shift",
                ...
                internship_duration: "time_period",
            };

            const field = keyMap[normalizedKey] || normalizedKey;
            console.log(
                `📝 Field mapping: "${key}" → normalized: "${normalizedKey}" → field: "${field}" | value:`,
                value,
            );

            if (field === "primary_skills") {
                // Handle primary skills - can be array or comma-separated string
                let skillsArray = Array.isArray(value)
                    ? value
                    : typeof value === "string"
                      ? value
                            .split(",")
            ...
            } else if (field === "secondary_skills") {
                // Handle secondary skills - can be array or comma-separated string
                let skillsArray = Array.isArray(value)
                    ? value
                    : typeof value === "string"
                      ? value
                ...
                setSecondarySkills(formattedSkills);
                setResetSecondary(true);
            } else if (field === "job_locations") {
                // Handle locations - can be array or comma-separated string
                let locationsArray = Array.isArray(value)
                    ? value
                    : typeof value === "string"
                      ? value
                            .split(",")
                ...
                const formattedLocs = locationsArray.map((loc) =>
                    typeof loc === "string"
                        ? { location: loc, job_type: "office", positions: 1 }
                        : loc,
                );
                setLocationData(formattedLocs);
                setResetLocation(true);
            } else if (field === "experience_min") {
                expUpdate[0] = parseFloat(value);
                hasExpUpdate = true;
                console.log(`📊 Set experience_min: ${expUpdate[0]}`);
            } else if (field === "experience_max") {
                expUpdate[1] = parseFloat(value);
                hasExpUpdate = true;
                console.log(`📊 Set experience_max: ${expUpdate[1]}`);
            } else if (field === "ctc_min") {
                ctcUpdate[0] = parseFloat(value);
                hasCtcUpdate = true;
                console.log(`💰 Set ctc_min: ${ctcUpdate[0]}`);
            } else if (field === "ctc_max") {
                ctcUpdate[1] = parseFloat(value);
                hasCtcUpdate = true;
                console.log(`💰 Set ctc_max: ${ctcUpdate[1]}`);
            } else if (field === "ctc" || field === "salary") {
                // Backend sends ctc as [min, max] array or {min, max} object
                console.log(
                    `💰 Processing CTC field, type: ${typeof value}, isArray: ${Array.isArray(value)}, value:`,
                    value,
                );
                if (Array.isArray(value)) {
                    // ARRAY FORMAT: Backend sends ctc as [min, max]
                    if (value.length >= 2) {
                        ctcUpdate = [
                            parseFloat(value[0]),
                            parseFloat(value[1]),
                        ...
                        hasCtcUpdate = true;
                        console.log(
                            `  ↳ Set CTC (array): [${ctcUpdate[0]}, ${ctcUpdate[1]}]`,
                        );
                        ...
                        hasCtcUpdate = true;
                        console.log(
                            `  ↳ Set CTC (single array): [${ctcUpdate[0]}, ${ctcUpdate[1]}]`,
                        );
                    }
                } else if (typeof value === "object" && value !== null) {
                    // OBJECT FORMAT: {min, max}
                    const min = value.min || value.minimum || value.ctc_min;
                    const max = value.max || value.maximum || value.ctc_max;
                    if (min !== undefined) {
                        ctcUpdate[0] = parseFloat(min);
                        hasCtcUpdate = true;
                        console.log(`  ↳ Set CTC min: ${ctcUpdate[0]}`);
                    }
                    if (max !== undefined) {
                        ctcUpdate[1] = parseFloat(max);
                        hasCtcUpdate = true;
                        console.log(`  ↳ Set CTC max: ${ctcUpdate[1]}`);
                    }
                } else if (typeof value === "number") {
                    ctcUpdate = [value, value];
                    hasCtcUpdate = true;
                    console.log(`  ↳ Set CTC (number): [${value}, ${value}]`);
                    ...
                    if (numbers && numbers.length >= 2) {
                        ...
                        hasCtcUpdate = true;
                        console.log(
                            `  ↳ Set CTC (string range): [${ctcUpdate[0]}, ${ctcUpdate[1]}]`,
                        );
                        ...
                        hasCtcUpdate = true;
                        console.log(
                            `  ↳ Set CTC (string single): [${ctcUpdate[0]}, ${ctcUpdate[1]}]`,
                        );
                    } else {
                        console.warn(
                            `  ⚠️ Could not parse CTC string: "${value}"`,
                        );
                    }
                } else {
                    console.warn(
                        `  ⚠️ Unexpected CTC value type: ${typeof value}`,
                    );
                }
            } else if (
                ...
                console.log(
                    `📊 Processing experience field, type: ${typeof value}, isArray: ${Array.isArray(value)}, value:`,
                    value,
                );
                if (Array.isArray(value)) {
                    // ARRAY FORMAT: Backend sends experience as [min, max]
                    if (value.length >= 2) {
                        expUpdate = [
                            parseFloat(value[0]),
                            parseFloat(value[1]),
                        ...
                        hasExpUpdate = true;
                        console.log(
                            `  ↳ Set experience (array): [${expUpdate[0]}, ${expUpdate[1]}]`,
                        );
                    } else if (value.length === 1) {
                        expUpdate = [
                            parseFloat(value[0]),
                        ...
                        hasExpUpdate = true;
                        console.log(
                            `  ↳ Set experience (single array): [${expUpdate[0]}, ${expUpdate[1]}]`,
                        );
                    }
                } else if (typeof value === "object" && value !== null) {
                    // OBJECT FORMAT: {min, max}
                    const min =
                        value.min || value.minimum || value.experience_min;
                    const max =
                        value.max || value.maximum || value.experience_max;
                    if (min !== undefined) {
                        expUpdate[0] = parseFloat(min);
                        hasExpUpdate = true;
                        console.log(`  ↳ Set experience min: ${expUpdate[0]}`);
                    }
                    if (max !== undefined) {
                        expUpdate[1] = parseFloat(max);
                        hasExpUpdate = true;
                        console.log(`  ↳ Set experience max: ${expUpdate[1]}`);
                    }
                } else if (typeof value === "number") {
                    expUpdate = [value, value];
                    hasExpUpdate = true;
                    console.log(
                        `  ↳ Set experience (number): [${value}, ${value}]`,
                    );
                } else if (typeof value === "string") {
                    // Parse string values like "3-5", "5+", "5"
                    const numbers = value.match(/(\d+(\.\d+)?)/g);
                    if (numbers && numbers.length >= 2) {
                        expUpdate = [
                            parseFloat(numbers[0]),
                        ...
                        hasExpUpdate = true;
                        console.log(
                            `  ↳ Set experience (string range): [${expUpdate[0]}, ${expUpdate[1]}]`,
                        );
                    } else if (numbers && numbers.length === 1) {
                        expUpdate = [
                            parseFloat(numbers[0]),
                        ...
                        hasExpUpdate = true;
                        console.log(
                            `  ↳ Set experience (string single): [${expUpdate[0]}, ${expUpdate[1]}]`,
                        );
                    } else {
                        console.warn(
                            `  ⚠️ Could not parse experience string: "${value}"`,
                        );
                    }
                } else {
                    console.warn(
                        `  ⚠️ Unexpected experience value type: ${typeof value}`,
                    );
                }
            } else if (field === "experience_min") {
                expUpdate[0] = parseFloat(value);
                ...
                hasExpUpdate = true;
                console.log(`📊 Set experience_max: ${expUpdate[1]}`);
            } else if (field === "notice_period") {
                // Map AI labels to internal form values
                const val = String(value).toLowerCase();
                let mappedValue = value;
                if (val.includes("serving")) mappedValue = "serving_notice";
                else if (val.includes("need") || val.includes("serve"))
                    mappedValue = "need_to_serve_notice";
                else if (val.includes("immediate"))
                    mappedValue = "immediate_joining";

                setValue("notice_period", mappedValue, {
                    shouldValidate: true,
                });
                updates["notice_period"] = mappedValue;
            } else if (field === "rotational_shift") {
                const isTrue =
                    String(value).toLowerCase() === "yes" ||
                    String(value).toLowerCase() === "true";
                setValue("rotational_shift", isTrue, { shouldValidate: true });
                updates["rotational_shift"] = isTrue;
            } else if (field === "working_days_per_week") {
                const num = parseInt(value);
                if (!isNaN(num)) {
                    setValue("working_days_per_week", num, {
                        shouldValidate: true,
                    });
                    updates["working_days_per_week"] = num;
                }
            } else if (field === "job_description") {
                // Convert newlines to HTML for ReactQuill
                const htmlValue = String(value).replace(/\n/g, "<br>");
                setValue("job_description", htmlValue, {
                    shouldValidate: true,
                });
                updates["job_description"] = htmlValue;
                console.log(
                    `✅ Set job_description (HTML formatted):`,
                    htmlValue.substring(0, 100) + "...",
                );
            } else if (field === "job_department") {
                // Explicit handler for job department
                const deptValue = String(value).trim();
                if (deptValue) {
                    setValue("job_department", deptValue, {
                        shouldValidate: true,
                    });
                    updates["job_department"] = deptValue;
                    console.log(`✅ Set job_department:`, deptValue);
                } else {
                    console.warn(`⚠️ Empty value for job_department`);
                }
            } else if (field === "industry") {
                // Explicit handler for industry
                const industryValue = String(value).trim();
                if (industryValue) {
                    setValue("industry", industryValue, {
                        shouldValidate: true,
                    });
                    updates["industry"] = industryValue;
                    console.log(`✅ Set industry:`, industryValue);
                } else {
                    console.warn(`⚠️ Empty value for industry`);
                }
            } else {
                // Handle regular fields - catch-all for unmapped fields
                try {
                    setValue(field, value, { shouldValidate: true });
                    updates[field] = value;
                    console.log(
                        `✅ Set field "${field}" (fallback handler) =`,
                        value,
                    );
                } catch (error) {
        ...
        // Apply accumulated range updates
        if (hasExpUpdate) {
            setValue("years_of_experience", expUpdate, {
                shouldValidate: true,
            });
            updates["years_of_experience"] = expUpdate;
            console.log(`✅ FINAL: Set years_of_experience =`, expUpdate);
        }
        if (hasCtcUpdate) {
            setValue("ctc", ctcUpdate, { shouldValidate: true });
            updates["ctc"] = ctcUpdate;
            console.log(`✅ FINAL: Set ctc =`, ctcUpdate);
        }
        ...
        if (Object.keys(updates).length > 0) {
            ...
            console.log(
                `✅ AI Chatbot - Synced ${Object.keys(updates).length} fields:`,
                Object.keys(updates),
            );
        }

        console.log(
            "✅ AI Chatbot - COMPLETE: Form fill finished successfully!",
        );
    };
    ...
    const handleAIResponse = (data) => {
        ...
        try {
            // Remove potential markdown code blocks if AI fails to follow strict JSON mode
            const cleanedJson = responseText
                .replace(/```json\n?|```/g, "")
                .trim();
            parsedData = JSON.parse(cleanedJson);
        } catch (error) {
            console.error("AI JSON Parse Error:", error, responseText);
            addAIMessage(
                "I'm sorry, I encountered an issue processing the data. Let me try again or please provide the details more simply.",
            );
            setIsAIProcessing(false);
            return;
        }
        ...
        switch (action) {
            case "extract":
                ...
                if (extracted_data) {
                    addFormPreviewMessage(extracted_data);
                    setExtractedData(extracted_data);
                    setConversationState("confirming");

                    addConfirmationMessage(
                        message || "Should I add these details to the form?",
                        () => handleConfirmation(true, extracted_data),
                        () => handleConfirmation(false, extracted_data),
                    );
                } else if (message) {
            ...
            case "ask_field":
                // Process any extracted data from user's answer to previous question
                if (extracted_data) {
                    console.log(
                        "📥 AI Chatbot - Processing extracted_data from individual question answer:",
                        extracted_data,
                    );
                    fillFormFields(extracted_data);
                }

                // Ask for missing field with options based on action_type
                setConversationState("asking");

                // ALWAYS use checkboxes for location fields - user requirement
                ...
                if (isLocationField) {
                    // Force checkbox UI for all location-related fields
                    console.log(
                        "📍 AI Chatbot - Using checkbox multi-select for location field:",
                        field_name,
                    );
                    addCheckboxOptionsMessage(
                        message || "Please select the job location(s):",
                        locationsList, // Always use the locations list from locations.jsx
                        field_name,
                    );
                } else if (action_type === "radio") {
                    addRadioOptionsMessage(
                        message,
                        field_options || [],
                        field_name,
                    );
                ...
                } else if (action_type === "search_select") {
                    addSearchSelectMessage(
                        message,
                        field_options || [],
                        field_name,
                    );
                } else if (action_type === "calendar") {
                    addCalendarMessage(message, field_name);
                } else {
            ...
            case "complete":
                ...
                addAIMessage(
                    message ||
                        "Thank you! All mandatory fields have been filled.",
                );
                setConversationState("idle");
                break;

            case "message":
                // Plain informative message
                if (message) {
                    addAIMessage(message);
                }
                break;
    ...
    const handleConfirmation = async (confirmed, data) => {
        const responseText = confirmed ? "Yes, add them" : "No, let me change";
        const currentData = data || extractedData;

        // Add user response locally and prepare for AI call
        const userMsg = {
            id: Date.now(),
        ...
        if (confirmed && currentData) {
            // Fill form fields first
            fillFormFields(currentData);

            // prepare full data to pass to callAI immediately to avoid stale state
            const updatedFormValues = {
                ...getValues(),
                ...currentData,
                primary_skills: currentData.primary_skills || primarySkills,
                secondary_skills:
                    currentData.secondary_skills || secondarySkills,
                job_locations:
                    currentData.job_locations ||
                    currentData.location ||
                    locationData,
            };

            // DON'T change state here - backend needs to receive 'confirming' state
            // to properly handle the "Yes, add them" response

            // Wait for state updates to settle then call AI with fresh context
            setTimeout(async () => {
                // Pass explicit values to callAI to avoid stale closure issues
                await callAI(responseText, newHistory, updatedFormValues);
            }, 100);
        } else {
            // Send "No" response to AI to ask what user wants to change
            await callAI(responseText, newHistory);
        }
    };

    // Helper to update specific state (skills, locations) when AI extracts or user selects
    const updateCustomState = (field, value) => {
        if (field === "primary_skills") {
            let skillsArray = Array.isArray(value) ? value : [value];
            const formatted = skillsArray.map((s) =>
                typeof s === "string"
                    ? {
                          skill_name: s,
                          metric_type: "rating",
            ...
            let locationsArray = Array.isArray(value) ? value : [value];
    ...
    const handleRadioSelection = async (value, fieldName) => {
        ...
        // Update custom state (locations/skills)
        updateCustomState(fieldName, value);

        // Update extracted data
        const updatedData = { ...extractedData, [fieldName]: value };
        ...
        // Send to AI to continue
        await callAI(value, newHistory, watch());
    };

    // Handle checkbox selections
    const handleCheckboxSelection = async (selectedOptions, fieldName) => {
        const textResponse = Array.isArray(selectedOptions)
        ...
        // Update custom state (locations/skills)
        updateCustomState(fieldName, selectedOptions);

        // Update extracted data
        const updatedData = { ...extractedData, [fieldName]: selectedOptions };
        setExtractedData(updatedData);

        // Update form field
        setValue(fieldName, selectedOptions, { shouldValidate: true });
        ...
        // Send to AI to continue
        await callAI(textResponse, newHistory, watch());
    };

    // Handle date selection
    const handleDateSelection = async (dateString, fieldName) => {
        // Add user response as message
        const userMsg = {
            id: Date.now(),
            type: MESSAGE_TYPES.USER,
        ...
        // Update custom state (locations/skills) if needed
        updateCustomState(fieldName, dateString);

        // Update extracted data
        const updatedData = { ...extractedData, [fieldName]: dateString };
        setExtractedData(updatedData);

        // Update form field
        setValue(fieldName, dateString, { shouldValidate: true });
        ...
        // Send to AI to continue
        await callAI(dateString, newHistory, watch());
    };

    // Call AI API
    const callAI = async (userMessage, customHistory, customFormData) => {
        setIsAIProcessing(true);

        try {
            // Simplify skills and locations to simple arrays for AI context
            // This ensures the AI recognizes them as "filled" based on its schema
            const simplifiedPrimary = (primarySkills || []).map((s) =>
                typeof s === "object" ? s.skill_name : s,
            );
            const simplifiedSecondary = (secondarySkills || []).map((s) =>
                typeof s === "object" ? s.skill_name : s,
            );
            const simplifiedLocations = (locationData || []).map((l) =>
                typeof l === "object" ? l.location : l,
            );

            // Prepare complete form data including simplified skills and locations
            const currentValues = getValues();
            const fullFormData = {
                ...currentValues,
                primary_skills: simplifiedPrimary,
                secondary_skills: simplifiedSecondary,
                job_locations: simplifiedLocations,
                ...(customFormData || {}),
            };

            // Special handling for customFormData skills/locations to ensure they are simplified strings
            if (customFormData?.primary_skills) {
                fullFormData.primary_skills = customFormData.primary_skills.map(
                    (s) => (typeof s === "object" ? s.skill_name : s),
                );
            }
            if (customFormData?.secondary_skills) {
                fullFormData.secondary_skills =
                    customFormData.secondary_skills.map((s) =>
                        typeof s === "object" ? s.skill_name : s,
                    );
            }
            if (customFormData?.job_locations) {
                fullFormData.job_locations = customFormData.job_locations.map(
                    (l) => (typeof l === "object" ? l.location : l),
                );
            } else if (customFormData?.location) {
                fullFormData.job_locations = customFormData.location.map((l) =>
                    typeof l === "object" ? l.location : l,
                );
            }

            const response = await fetch(
                `${apiurl}/ai/process-job-description/`,
                {
                    method: "POST",
                    headers: {
                    ...
                    body: JSON.stringify({
                        message: userMessage,
                        history: customHistory || messages,
    ...
    const handleSendMessage = async () => {
        ...
        await callAI(messageToSend, newHistory, watch());
    };
    ...
    const handleJDParse = (data) => {
        ...
        // 5. Locations Parsing
        const rawLocations = data.job_locations || data.location;
        if (rawLocations) {
            const locList = rawLocations
                .split(/,|\n|and/)
                .map((s) => s.trim())
                .filter((s) => s.length > 0);
                    ...
                    <div className={formItemsClass}>
                        ...
                        <div className="mb-4">
                            ...
                            <Controller
                                ...
                                control={control}
                                rules={{
                                    required: "Please enter job department",
                                }}
                                render={({ field, fieldState: { error } }) => (
                                    <>
                                        <Input
            ...
            <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end">
                ...
                >
                    ...
                    <div className="p-6 h-full overflow-y-auto no-scrollbar">
                        <div className="flex justify-center mt-12 text-indigo-700 p-4 rounded-xl mb-4 text-sm">
                            <svg
                                width="242"
                                height="180"
                                viewBox="0 0 242 180"
                ...
                {/* Chat Panel */}
                {isChatExpanded && (
                    <div className="fixed top-19 right-4 w-96 h-[700px] bg-white rounded-lg shadow-2xl z-50 flex flex-col border border-gray-200">
                        {/* Header */}
                        <div className=" text-black px-4 py-3 rounded-t-lg flex justify-between items-center">
                            <h3 className="font-semibold text-lg">
                                AI Assistant
                            </h3>
                            <button
                                onClick={() => setIsChatExpanded(false)}
                                className="text-black hover:bg-white/20 rounded px-2 py-1 transition-colors"
                            >
                                ✕
                            </button>
                        </div>
                            ...
                            {messages.length === 0 ? (
                                <div className="text-center text-gray-400 mt-8">
                                    ...
                                    <p className="text-sm mt-2">
                                        Describe the job you want to post, and
                                        I'll help fill out the form!
                                    </p>
                                    <p className="text-xs mt-4 text-gray-500">
                                        Example: "I need a senior React
                                ...
                                messages.map((msg) => {
                                    ...
                                    // Form Preview
                                    if (
                                        msg.type === MESSAGE_TYPES.FORM_PREVIEW
                                    ) {
                                        return (
                                            <div
                                                key={msg.id}
                                                className="flex justify-start"
                                            >
                                                <div className="bg-blue-50 border border-blue-200 rounded-lg p-3 max-w-[90%]">
                                    ...
                                    if (
                                        msg.type === MESSAGE_TYPES.CONFIRMATION
                                    ) {
                                        return (
                                            <div
                                                key={msg.id}
                                    ...
                                    // Radio Options Message
                                    if (
                                        msg.type === MESSAGE_TYPES.RADIO_OPTIONS
                                    ) {
                                        return (
                                            <RadioOptionItem
                                                key={msg.id}
                                    ...
                                    // Checkbox Options Message
                                    if (
                                        msg.type ===
                                        MESSAGE_TYPES.CHECKBOX_OPTIONS
                                    ) {
                                        return (
                                            <CheckboxOptionItem
                                                key={msg.id}
                                    ...
                                    if (
                                        msg.type === MESSAGE_TYPES.SEARCH_SELECT
                                    ) {
                                        return (
                                    ...
                                    // Calendar Message
                                    if (msg.type === MESSAGE_TYPES.CALENDAR) {
                                        return (
                                            <CalendarItem
                                                key={msg.id}
                                                msg={msg}
                ...
                {/* Floating Action Button - Hide when chat is expanded */}
                {!isChatExpanded && (
                    <button
                        onClick={() => setIsChatExpanded(!isChatExpanded)}
                        className="shadow-2xl hover:scale-105 transition-transform duration-200 border-none bg-transparent cursor-pointer p-0 flex items-center justify-center overflow-hidden"
                        style={{ width: "210px", height: "44px" }}
                    >
                        <svg
                            width="210"
                            height="44"
                            viewBox="57.5 57.5 210 44"
                            fill="none"
                            xmlns="http://www.w3.org/2000/svg"
                            ...
                            <defs>
                                ...
                                >
                                    ...
                                    <stop
                                        ...
                                        stopColor="#F9B2BC"
                                    />
                                    <stop offset="1" stopColor="#D142F5" />
                                </linearGradient>
                                <linearGradient
